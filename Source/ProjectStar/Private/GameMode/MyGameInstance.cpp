...
void UMyGameInstance::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			if (bWasSuccessful)
			{
				UGameplayStatics::OpenLevel(GetWorld(), "", true);
			}
		}
	}
}
...
void UMyGameInstance::OnFindSessionResult(const TArray<FBlueprintSessionResult>& SessionResults)
{
}

void UMyGameInstance::OnFindSessionResult_Implementation(const TArray<FBlueprintSessionResult>& SessionResults)
{
}
